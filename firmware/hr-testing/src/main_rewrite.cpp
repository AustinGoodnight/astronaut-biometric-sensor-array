...
// Fixed LED current. 0x2F ~ 9.4 mA (was 0x1F ~ 6.2 mA; raised to improve SNR on a weak pulse).
// Aim for finger DC roughly 30-70% of 262143; if IR DC on a finger exceeds ~200k, back this off.
const byte LED_CURRENT = 0x2F;
...
void tryInitSensor() {
  ...
  particleSensor.setup(LED_CURRENT, 4, 2, 400, 411, 4096);
...
void onTick(long irRaw) {
  ...
  logLine("status: ir=" + String(irRaw) + " avgBpm=" + bpmText + " led=" + String(LED_CURRENT) +
